...
def main(run_date=None, input_comment=None, load_cache=True):
    ...
    for target_item in target_size_elements:
        ...
        # trendAnalysisBlackBox.analyze_trend(trendConfig.config, param, target_item, incr_df, False, True)
        trendAnalysisFromTodayNew.analyze_trend(trendConfig.config, param, target_item, incr_df, False, True)
    ...
    for target_item in target_size_elements:
        param["target_size"] = target_item
        param["comment"]='RANDOM FIXED New baseline with 40 features= ' + str(target_item)
        # trendAnalysisBlackBox.analyze_trend(trendConfig.config, param, target_item, incr_df, True, True)
        trendAnalysisFromTodayNew.analyze_trend(trendConfig.config, param, target_item, incr_df, True, True)
