...
from DateAwareCache import global_cache
...
#
def fetch_feed(base_url: str, params: dict[str, str], config, globle_params: dict[str, str]):
    ...
    hit, ts = global_cache.get(symbol, globle_params["end_date"])
    if hit is False:
        # get the data and put it in cache
        ts = TimeSeries(key=config["alpha_vantage"]["key"])
        global_cache.put(symbol, globle_params['end_date'], ts)
    else:
        print('>> Cache hit for '+symbol + ' date = '+globle_params['end_date'])
    ...
    if globle_params.get('end_date') is not None:
        if df.iloc[-1]['date'] < globle_params['end_date']:
            currentTime = datetime.now().strftime('%Y-%m-%d %H:%M')
            print("At " + currentTime + " - No data for " + symbol + " after " + globle_params['end_date'])
            exit()
    return df
...
def fetch_all_data(config, param):
    ...
    df = fetch_feed(base_url, local_params, config, param)
    ...
    hit, response = global_cache.get('FINRA_debit', param['end_date'])
    if hit is False:
        # get the data and put it in cache
        try:
        ...
        global_cache.put('FINRA_debit', param['end_date'], response)
    else:
        print('>> Cache hit for FINRA_debit, date = '+param['end_date'])
    ...
    if symbol == 'NVDA' or symbol == 'SMCI' or symbol == 'CRDO' or symbol == 'TSM':
        print('>Get AMD data')
        ts = TimeSeries(key=config["alpha_vantage"]["key"])
    ...
    ti = TechIndicators(key=config["alpha_vantage"]["key"])
    if param.get('bband_time_period') is not None:
        time_period = param['bband_time_period']
    else:
    ...
    if 'cp_sentiment_ratio' in param['selected_columns']:
        ...
        if os.path.isfile(file_path):
            ...
            if not missing_dates.empty:
                ...
                # result_df = get_historical_cp_ratios_with_sentiments(symbol, missing_dates, api_key)
                result_df = get_historical_cp_ratios_with_sentiments_with_retry(symbol, missing_dates, api_key)
