...
def download_data(config, param):
    ...
    df['Volume_Oscillator'] = df['Fast_EMA'] - df['Slow_EMA']
...
def make_prediciton( model, raw_df: DataFrame, param: dict[str], currentDateTime, symbol):
    ...
    # input_tensor = input_tensor.unsqueeze(0)  ### This is commented out because leaving it in cause problem during

    model.eval()  # Set to evaluation mode

    with torch.no_grad():  # Ensure no gradient is computed
        prediction = model(input_tensor)
        print("Prediction shape:", prediction.shape)    # DEBUG
...
def load_data_to_cache(config: dict[str, str], param: dict[str]):
    df, num_data_points, display_date_range = download_data(config, param)    
...
def analyze_trend( config: dict[str, str], param: dict[str], current_day_offset: str, incr_df: DataFrame, turn_random_on: bool, use_cached_data: bool):
    ...
    class_weights = compute_class_weight(class_weight='balanced', classes=num_labels, y=train_labels)
    class_weights_dict = {0: class_weights[0], 1: class_weights[1], 2: class_weights[2] * param['down_weight']}  # use the down weighting param passed in
    ...
    # Define loss function (SEE BELOW)
    ...
    # torch.save(model, model_name)     # do not save for now to save space until we want to do inference
    ...
    for i in range(n_classes):
        ...
        roc_auc_scores[i] = auc_score  # Store the AUC score with the class label as the key
